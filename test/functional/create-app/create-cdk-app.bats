...
setup () {
    ...
    deleteEnvHeadlessFileName="/tmp/delete-cdk-app-env-headless-input.json"
...
@test "create cdk app locally and deploy nothing" {
...
EOF
    ...
    # cat "$generatedAppEnvDir/.choice-cache.json" >&3
    ...
    assert [ "$cacheJsonDeployTfBackend" == "n" ]
    defaultCreatedBy="$(jq -r '.defaultCreatedBy' "$generatedAppEnvDir/.choice-cache.json")"
    ...
    assert_exists "$iacRootsDir/$firstIacModuleName/package.json"
    ...
    assert_output --partial "deploy-cdk2-bootstrap-cf-stack"
    assert_output --partial "destroy-cdk2-bootstrap-cf-stack"
    assert_output --partial "diff-$firstIacModuleName"
    ...
    # CDK-specific verifications:
    assert_output --partial "runcdk:"
...
# Test that deploys the CDK bootstrap stack after app creation and also tests saving environment configs
# in the SSM Parameter Store and creating and deleting the app stack
@test "create cdk app and save env configs in SSM and run cdk bootstrap" {
...
EOF
    ...
    # Begin CDK Bootstrap Deployment assertions
    assert_output --partial "Finished deploying CDK v2 bootstrap CloudFormation stack to account $realAwsAccountId region $defaultRegion"
    assert_output --partial "Finished deploying CDK v2 bootstrap CloudFormation stack to account $realAwsAccountId region $secondaryRegion"

    # Begin Remote SSM Storage assertions
    assert_output --partial "Done posting environment variables to AWS SSM Parameter Store"

    cd "$generatedAppDir"

    # Deploy the app

    echo "      Deploying the example CDK module" >&3
    make deploy-example
    run cat "$generatedAppEnvDir/.log.txt"
    # echo -e $output >&3
    assert_output --partial "Finished executing \"cdk deploy\" on ${appParentDir}${gitProjectName}/iac/roots/${firstIacModuleName}"

    # Perform cleanup on created resources

    echo "      Destroying the example CDK module" >&3
    make destroy-example
    run cat "$generatedAppEnvDir/.log.txt"
    # echo -e $output >&3
    assert_output --partial "Finished executing \"cdk destroy\" on ${appParentDir}${gitProjectName}/iac/roots/${firstIacModuleName}"

    # Test that the environment configs stored on the SSM Parameter store can be retrieved
    # Make a backup copy of the original environment json file for later use
    cp "$generatedAppEnvDir/.environment-${envName}.json" "$generatedAppEnvDir/.environment-${envName}-original.json"
    echo "      Downloading $envName environment configs from the SSM Parameter Store" >&3
    run "$generatedAppEnvDir/utility-functions.sh" pull_env_vars "$envName" "ssm"
    ...
    cat <<EOF > "$deleteEnvHeadlessFileName"
    {
        "deleteEnv": "$envName",
        "deleteRemoteEnv": "y",
        "deleteCicdCloudResources": "n",
        "deleteCdk2BootstrapStack": "n",
        "deleteConfig": "n"
    }
EOF
    ...
    assert_output --partial "Deleting SSM variables with path /${appName}/remoteVars/$envName/ from region $defaultRegion"
