...
setup () {
    ...
    createAppIacModuleInputFileName="/tmp/create-tf-app-iac-module-headless-input.json"
...
EOF
    ...
    run create_tf_app_iac_module "$createAppIacModuleInputFileName"

    assert_exists "$generatedAppDir/$newIacModuleNestedPath/$newIacModuleName"
    assert_exists "$generatedAppDir/$newIacModuleNestedPath/$newIacModuleName/backend.tf"

    run cat "$generatedAppDir/Makefile"
    assert_output --partial "plan-$newIacMakeModuleName"
