...
setup_file () {
    ...
    appParentDir="/tmp/"
    gitProjectName="bats-cf-local-deploy-nothing"
...
EOF
    ...
    run create_cf_app "$inputFileName"
...
setup () {
    ...
    createAppIacModuleInputFileName="/tmp/create-cf-app-iac-module-headless-input.json"
...
EOF
    ...
    run create_cf_app_iac_module "$createAppIacModuleInputFileName"

    assert_exists "$generatedAppDir/$newIacModuleNestedPath/$newIacModuleName"
    assert_exists "$generatedAppDir/$newIacModuleNestedPath/$newIacModuleName/cf.yml"
