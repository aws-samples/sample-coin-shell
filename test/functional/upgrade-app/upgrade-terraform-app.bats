...
@test "Terraform app upgrade should not modify files that may have changed during app development" {
...
EOF
    ...
    assert_exists "iac/roots/$firstIacModuleName/backend.tf"
    echo "bats-upgrade-marker" >> "iac/roots/$firstIacModuleName/backend.tf"
    ...
    run get_last_line_of_file "iac/roots/$firstIacModuleName/backend.tf"
    assert [ "$output" == "bats-upgrade-marker" ]
